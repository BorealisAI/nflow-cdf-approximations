...
class ActNorm(AffineConstantFlow):
    ...
    def forward(self, x):
        ...
        # print(self.training, x.shape[0])
        if not self.data_dep_init_done:
            print(f"data dep init with {x.shape[0]} samples")
...
class AffineHalfFlow(nn.Module):
    ...
    def __init__(self, dim, parity, net_class=MLP, nh=24, scale=True, shift=True):
        ...
        self.net = net_class(self.dim , 2*self.dim , nh, activation=nn.Softplus)
    
    def forward(self, x):
        self.mask = self.mask.to(x)
        s, t = self.net(x * self.mask).split(self.dim, dim=1)
        s = torch.tanh(s)
        scale = torch.exp(s)
        shift = t
        ...
        z = x * masked_scale + masked_shift
        logdet = torch.sum(torch.log(masked_scale).view(scale.shape[0], -1), 1)

        return z, logdet
    
    def backward(self, z):
        self.mask = self.mask.to(z)
        s, t = self.net(z * self.mask).split(self.dim, dim=1)
        s = torch.tanh(s)
        scale = torch.exp(s)
        shift = t
...
class MAF(nn.Module):
    ...
    def __init__(self, dim, parity, net_class=ARMLP, nh=24):
        super().__init__()
        self.dim = dim
        self.net = net_class(dim, dim*2, nh, act=nn.Softplus)
        self.parity = parity
    
    def forward(self, x):
        # here we see that we are evaluating all of z in parallel, so density estimation will be fast
        s, t = self.net(x).split(self.dim, dim=1)
        s = torch.tanh(s)
        z = x * torch.exp(s) + t
    ...
    def backward(self, z):
        ...
        x = torch.zeros_like(z)
        log_det = torch.zeros(z.size(0)).to(x)
        z = z.flip(dims=(1,)) if self.parity else z
        for i in range(self.dim):
...
class Invertible1x1Conv(nn.Module):
    ...
    def _assemble_W(self):
        ...
        L = torch.tril(self.L, diagonal=-1) + torch.diag(torch.ones(self.dim)).to(self.L)
        U = torch.triu(self.U, diagonal=1)
        W = self.P @ L @ (U + torch.diag(self.S))
        return W

    def forward(self, x):
        W = self._assemble_W().to(x.device)
        z = x @ W
