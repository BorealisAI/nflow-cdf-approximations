...
class AutoencoderDiffEqNet(nn.Module):
    ...
    def __init__(self, hidden_dims, input_shape, strides, conv, layer_type="concat", nonlinearity="softplus"):
        ...
        assert nonlinearity in ("tanh", "relu", "softplus", "elu", "swish")

        self.nonlinearity = {"tanh": F.tanh, "relu": F.relu, "softplus": F.softplus, "elu": F.elu, "swish": F.silu}[nonlinearity]
        if conv:
            assert len(strides) == len(hidden_dims) + 1
            base_layer = {
                "ignore": diffeq_layers.IgnoreConv2d,
...
class ODEfunc(nn.Module):
    ...
    def forward(self, t, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            # Hack on top of hack to use exact trace in test
            if not self.training:# and dy.view(dy.shape[0], -1).shape[1] == 2:
                ...
                divergence = divergence_bf(dy, y).view(batchsize, 1)
                # sqjacnorm = 0
                _, sqjacnorm = self.divergence_fn(dy, y, e=self._e)
